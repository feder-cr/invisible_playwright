...
substitution applies in full. A solid red rectangle comes back solid red. A real
...
input from the actual rendering pipeline underneath. Additive noise on top of a real
...
matter what happens upstream. It is also, necessarily, an enumeration problem - the
...
**Can canvas and WebGL fingerprints be made identical across operating systems?**
Yes, if the interception happens at every point content can read the result rather
...
a Firefox patched at the C++ level, on making a seed mean the same machine regardless
of what it's actually running on.*
