...
<script type="application/ld+json">
{
  ...
  "itemListElement": [
    ...
    },
    {
      "@type": "ListItem",
      "position": 4,
      "name": "Client Hints and Sec-Fetch: headers that must agree"
    }
  ]
}
</script>
...
Two families of request headers are generated by the browser from its own internal state,
not by the page and not by anything you configure. That makes them awkward to fake and
very cheap to check, which is a combination detectors like.
...
A separate family, present in every modern browser including Firefox, and much less
discussed.
